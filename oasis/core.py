...
class OasisOrchestrator:
    ...
    def setup_argument_parser(self):
        ...
        analysis_group.add_argument(
            "-at",
            "--analyze-type",
            choices=["standard", "adaptive", "collaborative"],
        ...
        agent_group.add_argument(
            "--multi-agent",
            action="store_true",
            default=DEFAULT_ARGS["MULTI_AGENT"],
            help="Enable multi-agent collaborative analysis",
        )
        agent_group.add_argument(
            "--agent-collaboration",
            action="store_true",
            default=DEFAULT_ARGS["AGENT_COLLABORATION"],
        ...
        mcp_group.add_argument(
            "--mcp-tools",
            action="store_true",
            default=DEFAULT_ARGS["MCP_TOOLS_ENABLED"],
        ...
        cache_group.add_argument(
            ...
            "--cache-days",
            type=int,
            default=DEFAULT_ARGS["CACHE_DAYS"],
        ...
        web_group.add_argument(
            "-w", "--web", action="store_true", help="Serve reports via a web interface"
        )
        web_group.add_argument(
            "-we",
            "--web-expose",
        ...
        logging_group.add_argument(
            "-s", "--silent", action="store_true", help="Disable all output messages"
        )
        ...
        special_group.add_argument(
            "-V", "--version", action="store_true", help="Show OASIS version and exit"
        )
    ...
    def check_dependencies(self):
        ...
        if missing_deps:
            logger.warning(
                f"Missing dependencies for A2A/MCP: {', '.join(missing_deps)}"
            )
    ...
    def run_analysis_mode(self, main_models, scan_model, vuln_mapping):
        ...
        vulnerabilities, invalid_tags = AgentOrchestrator.get_vulnerabilities_to_check(
            self.args, vuln_mapping
        )
        if invalid_tags:
            return False
        ...
        if self.args.multi_agent and not self.legacy_mode:
            ...
            return self._run_multi_agent_analysis(
                vulnerabilities, main_models, scan_model
            )
        else:
    ...
    def _run_multi_agent_analysis(self, vulnerabilities, main_models, scan_model):
        ...
        try:
            ...
            self.report.generate_executive_summary(
                all_results, "Multi-Agent Collaborative"
            )

            self.report.report_generated(
                report_type="Multi-Agent Security", report_structure=True
            )
            return True
    ...
    def _run_legacy_analysis(self, vulnerabilities, main_models, scan_model):
        ...
        for i, main_model in enumerate(main_models):
            ...
            try:
                ...
                logger.exception(
                    f"Error during legacy analysis with {main_model}: {str(e)}"
                )
                continue
    ...
    def handle_audit_mode(self, vuln_mapping):
        ...
        embedding_analyzer = EmbeddingAnalyzer(
            self.embedding_manager, self.ollama_manager
        )

        # Analyze all vulnerabilities
        analyzer_results = embedding_analyzer.analyze_all_vulnerabilities(
            vulnerabilities
        )
        ...
        self.report.create_report_directories(
            self.args.input_path, models=[self.report.current_model]
        )
    ...
    def run(self, args=None):
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.exception(
                    f"Full error trace: {traceback.format_exc()}", exc_info=True
                )
            return 1
    ...
    def _init_oasis(self, args):
        ...
        # Check dependencies
        if not self.check_dependencies() and (
            self.args.multi_agent or self.args.mcp_tools
        ):
    ...
    def _init_ollama(self, ollama_url=None, check_embeddings=True):
        ...
        if self.args.chunk_size is None:
            self.args.chunk_size = self.ollama_manager.detect_optimal_chunk_size(
    ...
    def _init_mcp_tools(self):
        ...
        try:
            ...
            self.mcp_tools = MCPToolManager(
                config_file=self.args.mcp_config, enabled=self.args.mcp_tools
            )
    ...
    def _run_analysis_mode(self, vuln_mapping):
        ...
        selected_model_data = self.ollama_manager.select_analysis_models(
            self.args, available_models
        )
        ...
        if not main_models:
            logger.warning(
                "No main models were selected, using scan model for deep analysis as well"
            )
        ...
        display_main_models = ", ".join(
            [self.ollama_manager.get_model_display_name(m) for m in main_models]
        )

        if len(main_models) == 1 and scan_model == main_models[0]:
            logger.info(
                f"{MODEL_EMOJIS['default']}Using '{display_scan_model}' for both scanning and deep analysis"
            )
        else:
            logger.info(
                f"{MODEL_EMOJIS['default']}Using '{display_scan_model}' for scanning and {display_main_models} for deep analysis"
            )
        ...
        if self.args.multi_agent and self.agent_orchestrator:
            logger.info(
                f"Agent cache files: {self.agent_orchestrator.get_cache_info()}"
            )

        if self.args.mcp_tools and self.mcp_tools:
            logger.info(
                f"MCP tools used: {', '.join(self.mcp_tools.get_active_tools())}"
            )
    ...
    def _setup_logging(self):
        ...
        if self.args.silent:
            logs_dir = (
                Path(self.args.input_path).resolve().parent
                / REPORT["OUTPUT_DIR"]
                / "logs"
                if self.args.input_path
                else Path(REPORT["OUTPUT_DIR"]) / "logs"
            )
            logs_dir.mkdir(parents=True, exist_ok=True)
            log_file = (
                logs_dir / f"oasis_errors_{generate_timestamp(for_file=True)}.log"
            )
        else:
            log_file = None

        setup_logging(
            debug=self.args.debug, silent=self.args.silent, error_log_file=log_file
        )
