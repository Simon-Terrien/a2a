...
class MCPToolManager:
    ...
    def _init_semgrep_server(self):
        ...
        try:
            ...
            def validate_with_semgrep(
                file_path: str, vulnerability_type: str = ""
            ) -> Dict[str, Any]:
                """
    ...
    def _init_git_analyzer_server(self):
        ...
        try:
            ...
            def analyze_git_history(
                file_path: str, lines_of_interest: List[int] = None
            ) -> Dict[str, Any]:
                """
                ...
                try:
                    ...
                    if blame_result.returncode != 0:
                        return {
                            "error": "Git blame failed - not a git repository or file not tracked"
                        }
            ...
            def scan_dependencies(project_path: str) -> Dict[str, Any]:
                ...
                try:
                    ...
                    return {
                        ...
                        "scanned_files": [
                            str(f)
                            for f in [*requirements_files, package_json, pom_xml]
                            if f.exists()
                        ],
                    ...
                    return {
                        "error": f"Dependency scan error: {str(e)}",
                        "vulnerabilities": [],
                    }
    ...
    def start_servers(self) -> bool:
        ...
        for tool_name, server in self.tools.items():
            try:
                ...
                thread = threading.Thread(
                    target=run_server, args=(server, port), daemon=True
                )
                thread.start()
        ...
        if success_count > 0:
            logger.info(
                f"🔧 Started {success_count}/{len(self.tools)} MCP tool servers"
            )
            return True
        else:
    ...
    def _parse_git_blame(self, blame_output: str) -> Dict[str, Any]:
        ...
        blame_info = {"authors": {}, "commits": {}, "line_count": 0}
        ...
        try:
            ...
            for group_id, artifact_id, version in dependencies:
                for pattern, vuln_version in vulnerable_patterns:
                    if pattern in artifact_id.lower() and version.startswith(
                        vuln_version
                    ):
                        vulnerabilities.append(
                            {
