...
class Report:
    ...
    def create_report_directories(
        self, input_path: str | Path, sub_dir: str = None, models: List[str] = None
    ) -> Dict[str, Path]:
        """
    ...
    def generate_vulnerability_report(
        ...
        self._generate_and_save_report(
            output_files, report, report_type="Vulnerability"
        )

        return output_files

    def _generate_and_save_report(
        self, output_files, report_content, report_type: str = None
    ):
        """
        ...
        logger.debug("--------------------------------")
        logger.debug(
            f"Generating {report_type} report for {', '.join(self.output_format)}, please wait..."
        )
    ...
    def generate_executive_summary(
        self, all_results: Dict[str, List[Dict]], model_name: str
    ) -> Dict[str, Path]:
        """
        ...
        for severity in ["High", "Medium", "Low"]:
            if severity_groups[severity]:
                ...
                for finding in sorted(
                    severity_groups[severity], key=lambda x: x["score"], reverse=True
                ):
        ...
        # Generate and save report
        self._generate_and_save_report(
            output_files, report, report_type="Executive Summary"
        )
    ...
    def convert_to_all_formats(self, markdown_file: Path) -> Dict[str, Path]:
        ...
        try:
            ...
            # Convert to PDF
            try:
                HTML(string=rendered_html, media_type="print").write_pdf(
                    output_files["pdf"]
                )
            except Exception as e:
                logger.exception(
                    f"PDF conversion failed for {markdown_file.name}: {e.__class__.__name__}: {str(e)}"
                )
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"HTML content causing PDF conversion failure (first 500 chars): {rendered_html[:500]}"
                    )
            ...
            logger.exception(
                f"Error converting {markdown_file.name} to other formats: {e.__class__.__name__}: {str(e)}"
            )
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("Full error:", exc_info=True)

            return {"md": markdown_file, "pdf": None, "html": None}
    ...
    def get_output_directory(
        self, input_path: str | Path, base_reports_dir: Path
    ) -> Path:
        """
