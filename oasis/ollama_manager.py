...
class OllamaManager:
    ...
    def get_available_models(self, show_formatted: bool = False) -> List[str]:
        ...
        try:
            ...
            if show_formatted and model_names:
                ...
                logger.info("\nAvailable models:")
                for i, (model_name, formatted_model) in enumerate(
                    zip(model_names, self.formatted_models), 1
                ):
            ...
            logger.warning(
                f"Using default model list: {', '.join(self.default_models)}"
            )
    ...
    def _detect_optimal_chunk_size(self, model):
        ...
        logger.warning(
            f"Could not detect context length for {model}, using default size: {MAX_CHUNK_SIZE}"
        )
        return MAX_CHUNK_SIZE
    ...
    def select_models(
        ...
        max_models: int = None,
        msg: str = "",
        recommend_lightweight: bool = False,
    ) -> List[str]:
        """
        ...
        if recommend_lightweight and (
            lightweight_models := self._filter_lightweight_models(available_models)
        ):
            logger.info(
                f"Filtering models to display only lightweight models (< 10B parameters): {len(lightweight_models)} models found."
            )
            if not lightweight_models:
                logger.warning(
                    "No lightweight models found, displaying all available models."
                )
            else:
        ...
        logger.info("\nAvailable models:")
        for i, (model_name, formatted_name) in enumerate(
            zip(available_models, formatted_models), 1
        ):
        ...
        try:
            ...
            while len(selected_models) < (max_models or len(available_models)):
                ...
                try:
                    selected_indices = [
                        int(idx.strip()) for idx in selection.split(",") if idx.strip()
                    ]
                    ...
                    if not all(
                        0 <= idx < len(available_models) for idx in selected_indices
                    ):
                        logger.error(
                            f"Invalid selection. Numbers must be between 1 and {len(available_models)}"
                        )
                        continue
                    ...
                    selected_models = [
                        available_models[idx] for idx in selected_indices
                    ]
    ...
    def format_model_display(self, model_name: str) -> str:
        ...
        try:
            ...
            return self._build_formatted_string(
                model_name, model_emoji, param_str, ctx_str, parent_info
            )
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(model_info, dict):
                        ...
                        if (
                            parameters == 0
                            and "modelinfo" in model_info
                            and model_info["modelinfo"]
                        ):
                            modelinfo = model_info["modelinfo"]
                            if (
                                isinstance(modelinfo, dict)
                                and "general.parameter_count" in modelinfo
                            ):
                        ...
                        details = model_info.details
                        if (
                            hasattr(details, "parameter_size")
                            and details.parameter_size
                        ):
                    ...
                    if (
                        parameters == 0
                        and hasattr(model_info, "modelinfo")
                        and model_info.modelinfo
                    ):
                        modelinfo = model_info.modelinfo
                        if (
                    ...
                    logger.debug(
                        f"Error parsing parameter information for {model}: {str(inner_e)}"
                    )
    ...
    def select_analysis_type(self, args) -> str:
        ...
        logger.info(
            "2. Adaptive - Multi-level analysis that adjusts depth based on risk assessment"
        )
    ...
    def select_analysis_models(self, args, available_models):
        ...
        if not hasattr(args, "models") or not main_models:
            ...
            if not (
                main_models := self.select_models(
    ...
    def ensure_model_available(self, model: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"🤖 Model {model} not found locally, pulling from Ollama library..."
            )
            ...
            try:
                        ...
                        if "status" in response:
                            ...
                            if "completed" in status:
                                if "completed" in response:
                                    ...
                                    pbar.update(
                                        pbar.total - pbar.n
                                    )  # Fallback update if no detailed progress available
                ...
                logger.error(
                    "Please check that the model name is correct and available from Ollama"
                )
                return False
            ...
            if isinstance(model_info, dict):
                ...
                if "details" in model_info and model_info["details"]:
                    ...
                    if isinstance(details, dict) and "parameter_size" in details:
                        ...
                        if isinstance(param_size, str) and "B" in param_size:
                            parameters = (
                                float(param_size.replace("B", "")) * 1_000_000_000
                            )
                        elif isinstance(param_size, str) and "M" in param_size:
    ...
    def _extract_parent_model_info(
        self, model_info: Any, default_emoji: str = "🤖 "
    ) -> str:
        """
        ...
        try:
            ...
            if isinstance(model_info, dict):
                if (
                    ...
                    and model_info["details"]
                    and isinstance(model_info["details"], dict)
                    and "parent_model" in model_info["details"]
                    and model_info["details"]["parent_model"]
                ):
            ...
            # Check for object format (older API)
                ...
                hasattr(model_info, "details")
                and model_info.details
                and hasattr(model_info.details, "parent_model")
                and model_info.details.parent_model
            ):
    ...
    def _get_model_emoji(model_name: str, default_emoji: str = "🤖 ") -> str:
        ...
        model_family = (
            model_parts[0] if len(model_parts) > 1 else None
        )  # potential family
        ...
        if best_match_length == 0:
            for model_id, emoji in MODEL_EMOJIS.items():
                ...
                if (
                    model_id in model_lower
                    or (model_family and model_id in model_family)
                    or any(model_id in family for family in model_families)
                ):
                    model_emoji = emoji
