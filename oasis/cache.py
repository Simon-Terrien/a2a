...
from .config import AnalysisMode, AnalysisType
...
class CacheManager:
    ...
    def __init__(
        ...
        for directory in list(self.standard_cache_dir.values()) + list(
            self.adaptive_cache_dir.values()
        ):
            directory.mkdir(exist_ok=True)

        # Initialize cache dictionaries for current session
        self.chunk_cache = {AnalysisType.STANDARD: {}, AnalysisType.ADAPTIVE: {}}
        self.scan_chunk_cache = {AnalysisType.STANDARD: {}, AnalysisType.ADAPTIVE: {}}
    ...
    def get_cache_path(
        self, file_path: str, mode: AnalysisMode, analysis_type: AnalysisType
    ) -> Path:
        """
        ...
        if action == "load":
            ...
            try:
                with open(cache_path, "rb") as f:
                    ...
                    logger.debug(
                        f"Loaded {mode.value} {analysis_type.value} chunk cache for {file_path}: {len(cache_dict[file_path])} entries"
                    )
                    return cache_dict[file_path]
            except Exception as e:
                logger.exception(
                    f"Error loading {mode.value} {analysis_type.value} chunk cache: {str(e)}"
                )
                cache_dict[file_path] = {}
                return {}
            ...
            try:
                ...
                with open(cache_path, "wb") as f:
                    ...
                    logger.debug(
                        f"Saved {mode.value} {analysis_type.value} chunk cache for {file_path}: {len(cache_dict[file_path])} entries"
                    )
            except Exception as e:
                logger.exception(
                    f"Error saving {mode.value} {analysis_type.value} chunk cache: {str(e)}"
                )
    ...
    def generate_cache_key(
        self, chunk: str, prompt: str, vuln_name: str, file_path: str = None
    ) -> str:
        """
    ...
    def clear_scan_cache(self, analysis_type: AnalysisType = None) -> None:
        ...
        try:
            ...
            if analysis_type is None:
                ...
                cache_dirs = [
                    (
                        self.standard_cache_dir[AnalysisMode.SCAN]
                        if AnalysisType.STANDARD in self.scan_chunk_cache
                        else None
                    ),
                    (
                        self.standard_cache_dir[AnalysisMode.DEEP]
                        if AnalysisType.STANDARD in self.chunk_cache
                        else None
                    ),
                    (
                        self.adaptive_cache_dir[AnalysisMode.SCAN]
                        if AnalysisType.ADAPTIVE in self.scan_chunk_cache
                        else None
                    ),
                    (
                        self.adaptive_cache_dir[AnalysisMode.DEEP]
                        if AnalysisType.ADAPTIVE in self.chunk_cache
                        else None
                    ),
                ]
            else:
    ...
    def validate_cache_expiration(self):
        ...
        try:
            ...
            all_cache_dirs = list(self.standard_cache_dir.values()) + list(
                self.adaptive_cache_dir.values()
            )
                ...
                for cache_file in cache_dir.glob("*.cache"):
                    ...
                    if cache_age.days > self.cache_days:
                        try:
                            ...
                            logger.warning(
                                f"Could not delete expired cache file {cache_file}: {e}"
                            )

            if expired_count > 0:
                logger.info(
                    f"Removed {expired_count} expired cache files older than {self.cache_days} days"
                )
