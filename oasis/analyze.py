...
class SecurityAnalyzer:
    def __init__(
        ...
        logger.info("\n")
        logger.info(
            f"{MODEL_EMOJIS['default']} Using {self.ollama_manager.get_model_display_name(self.scan_model)} for initial scanning and {self.ollama_manager.get_model_display_name(self.llm_model)} for deep analysis"
        )
        ...
        # Cache parameters
        self.clear_cache_scan = (
            args.clear_cache_scan if hasattr(args, "clear_cache_scan") else False
        )
        self.cache_days = (
            args.cache_days
            if hasattr(args, "cache_days")
            else DEFAULT_ARGS["CACHE_DAYS"]
        )
    ...
    def _get_vulnerability_details(
        ...
        if isinstance(vulnerability, dict):
            return (
                ...
                vulnerability.get("patterns", []),
                vulnerability.get("impact", ""),
                vulnerability.get("mitigation", ""),
            )
...
"""
    ...
    def _analyze_code_chunk(
        ...
        if self.cache_manager.has_caching_info(
            file_path, chunk_text, vuln_name, analysis_type
        ):
        ...
        try:
            ...
            response = self.client.chat(
                ...
                },  # Convert to milliseconds if API supports it
            )
            ...
            logger.exception(
                f"Error during chunk analysis with {model_display}: {str(e)}"
            )
            return f"Error during chunk analysis: {str(e)}"

    def _build_scan_prompt(
        self, vuln_name: str, vuln_desc: str, chunk_text: str
    ) -> str:
        """
...
"""
    ...
    def search_vulnerabilities(
        self,
        vulnerability: Union[str, Dict],
        threshold: float = DEFAULT_ARGS["THRESHOLD"],
        ...
        try:
            ...
            vuln_vector = self.embedding_manager.get_vulnerability_embedding(
                vulnerability
            )
            if not vuln_vector:
                logger.error(
                    f"Failed to get embedding for vulnerability type '{vuln_name}'. No embedding returned."
                )
                return []
            ...
            for file_path, data in self.code_base.items():
                if self.analyze_by_function:
                    ...
                    self._process_functions(
                        file_path, data, vuln_vector, threshold, results
                    )
                else:
    ...
    def analyze_vulnerability_adaptive(
        self, file_path: str, vulnerability: Union[str, Dict], threshold: float = 0.7
    ) -> str:
        """
    ...
    def process_analysis_with_model(self, vulnerabilities, args, report: Report):
        ...
        if adaptive:
            all_results = self.analysis_pipeline.perform_adaptive_analysis(
                vulnerabilities, args, report
            )
        else:
    ...
    def _perform_initial_scanning(self, vulnerabilities, args, main_pbar=None):
        ...
        logger.info(
            f"Using {self.ollama_manager.get_model_display_name(self.scan_model)} for initial scanning"
        )
        ...
        suspicious_files_by_vuln = self._identify_suspicious_files(
            vulnerabilities, args.threshold
        )
        ...
        with tqdm(
            total=len(suspicious_files_by_vuln),
            desc="Vulnerabilities analyzed (initial scan)",
            position=1,
            leave=False,
            disable=args.silent,
        ) as vuln_scan_pbar:
            for vuln_name, data in suspicious_files_by_vuln.items():
                ...
                with tqdm(
                    ...
                    desc=f"Scanning files for {vuln_name}",
                    position=2,
                    leave=False,
                    disable=args.silent,
                ) as file_pbar:
    ...
    def _scan_file_for_vulnerability(
        self, file_path, vuln, vuln_details, similarity_score, silent=False
    ):
        """
        ...
        self.suspicious_sections[(file_path, vuln_name)] = [
            idx for idx, _ in suspicious_chunks
        ]
    ...
    def _perform_deep_analysis(self, suspicious_data, args, report, main_pbar=None):
        ...
        logger.info(
            f"Using {self.ollama_manager.get_model_display_name(self.llm_model)} for deep analysis"
        )
        ...
        with tqdm(
            total=len(suspicious_files_by_vuln),
            desc="Vulnerabilities analyzed (deep analysis)",
            position=1,
            leave=False,
            disable=args.silent,
        ) as deep_vuln_pbar:
            for vuln_name, data in suspicious_files_by_vuln.items():
                ...
                detailed_results = self._analyze_vulnerability_deep(
                    vuln, vuln_name, all_suspicious_chunks, args.silent
                )
    ...
    def _analyze_vulnerability_deep(
        self, vuln, vuln_name, all_suspicious_chunks, silent=False
    ):
        """
        ...
        suspicious_files = [
            file_path
            for (file_path, vname), _ in all_suspicious_chunks.items()
            if vname == vuln_name
        ]
        ...
        with tqdm(
            total=len(suspicious_files),
            desc=f"Analyzing files for {vuln_name}",
            position=2,
            leave=False,
            disable=silent,
        ) as file_pbar:
    ...
    def _analyze_file_suspicious_chunks(
        self, file_path, suspicious_data, vuln, silent=False
    ):
        """
        ...
        vuln_name, vuln_desc, vuln_patterns, vuln_impact, vuln_mitigation = (
            self._get_vulnerability_details(vuln)
        )

        # Perform deep analysis on suspicious chunks
        analyses = []
        with tqdm(
            total=len(suspicious_chunks),
            desc=f"Chunks in {Path(file_path).name}",
            position=3,
            leave=False,
            disable=silent,
    ...
    def _identify_suspicious_files(self, vulnerabilities, threshold):
        ...
        with tqdm(
            total=len(vulnerabilities),
            desc="Searching for similarities by vulnerability",
            position=0,
            leave=False,
        ) as vuln_pbar:
            for vuln in vulnerabilities:
                ...
                logger.debug(
                    f"Found {len(filtered_results)} potentially vulnerable files for {vuln['name']}"
                )
    ...
    def get_vulnerabilities_to_check(args, vuln_mapping):
        ...
        if invalid_tags := [tag for tag in selected_tags if tag not in vuln_mapping]:
        ...
        try:
            ...
            if isinstance(file_vectors, list) and isinstance(file_vectors[0], list):
                highest_similarity = max(
                    calculate_similarity(vuln_vector, vec) for vec in file_vectors
                )
    ...
    def _extract_file_vectors(
        ...
        elif isinstance(embedding, list) and all(
...
class EmbeddingAnalyzer:
    ...
    def __init__(
        self, embedding_manager: EmbeddingManager, ollama_manager: OllamaManager
    ):
        """
    ...
    def generate_threshold_analysis(
        self, results: List[Dict], thresholds: List[float] = None
    ) -> List[Dict]:
        """
    ...
    def calculate_statistics(self, results: List[Dict]) -> Dict[str, float]:
        ...
        """
        if not results:
            return {"avg_score": 0, "median_score": 0, "max_score": 0, "min_score": 0}
    ...
    def analyze_all_vulnerabilities(
        ...
        for vuln in vulnerabilities:
            ...
            if console_output:
                self._print_vulnerability_analysis(
                    vuln_name, results, threshold_analysis, statistics
                )

        return all_results

    def generate_vulnerability_statistics(
    ...
    def _print_vulnerability_analysis(
        self,
        vuln_name: str,
        results: List[Dict],
        ...
        for analysis in threshold_analysis:
            ...
            logger.info(
                f"Threshold {threshold:.1f}: {matching_items:3d} items ({percentage:5.1f}%)"
            )
...
def analyze_item_parallel(args: tuple) -> Dict:
    ...
    try:
        ...
        vuln_response = client.embeddings(
            model=args.embedding_model, prompt=rich_prompt
        )
        ...
        elif isinstance(args.data.get("embedding"), list) and isinstance(
            args.data["embedding"][0], list
        ):
        ...
        similarity = calculate_similarity(vuln_response["embedding"], item_embedding)
...
class AdaptiveAnalysisPipeline:
    ...
    def _static_pattern_analysis(
        self, code_chunks: List[str], vuln_patterns: List[str]
    ) -> List[Tuple[int, str]]:
        """
        ...
        with tqdm(
            total=len(code_chunks), desc="Static pattern analysis", leave=False
        ) as pbar:
            for i, chunk in enumerate(code_chunks):
        ...
        logger.debug(
            f"Static analysis identified {len(suspicious_chunks)}/{len(code_chunks)} suspicious chunks"
        )
        return suspicious_chunks
    ...
    def _lightweight_model_scan(
        ...
        logger.debug(
            f"After lightweight scan: {len(final_suspicious_chunks)}/{len(code_chunks)} suspicious chunks"
        )
        return final_suspicious_chunks

    def _identify_context_sensitive_chunks(
        self, suspicious_chunks: List[Tuple[int, str]], vuln_name: str
    ) -> List[Tuple[int, str]]:
        """
        ...
        with tqdm(
            total=len(suspicious_chunks),
            desc="Identifying context-sensitive chunks",
            leave=False,
        ) as pbar:
        ...
        logger.debug(
            f"Identified {len(context_sensitive_chunks)}/{len(suspicious_chunks)} context-sensitive chunks"
        )
    ...
    def _medium_model_analysis(
        ...
        with tqdm(
            total=len(context_chunks), desc="Medium-depth analysis", leave=False
        ) as pbar:
...
"""
                ...
                try:
                    ...
                    if analysis_result and "RISK_SCORE:" in analysis_result:
                        ...
                        risk_score = int(
                            re.search(r"RISK_SCORE:\s*(\d+)", risk_line)[1]
                        )
    ...
    def _identify_high_risk_chunks(
        ...
        risk_map = {
            result["chunk_idx"]: result["risk_score"] for result in medium_results
        }
        ...
        logger.debug(
            f"Identified {len(high_risk_chunks)}/{len(suspicious_chunks)} high-risk chunks"
        )
    ...
    def _deep_model_analysis(
        self,
        file_path: str,
        high_risk_chunks: List[Tuple[int, str]],
        vuln_name: str,
        vuln_desc: str,
        vuln_patterns: List[str],
        vuln_impact: str,
        ...
        with tqdm(
            total=len(high_risk_chunks),
            desc="Deep analysis of high-risk chunks",
            leave=False,
        ) as pbar:
...
"""
    ...
    def _combine_adaptive_results(
        ...
        vulnerable_chunks = [
            ...
            if "No" not in result["analysis"]
            or "vulnerabilities found" not in result["analysis"]
        ]
...
"""
    ...
    def perform_adaptive_analysis(self, vulnerabilities, args, report):
        ...
        """
        logger.info(
            f"Using {self.ollama_manager.get_model_display_name(self.llm_model)} for adaptive analysis"
        )
        ...
        with tqdm(
            ...
            for vuln in vulnerabilities:
                ...
                results = self.analyzer.search_vulnerabilities(
                    vuln, threshold=args.threshold
                )
                filtered_results = [
                    (path, score) for path, score in results if score >= args.threshold
                ]
        ...
        if all_tasks:
            logger.info(
                f"Processing {len(all_tasks)} tasks with optimized model loading"
            )
        ...
        with tqdm(
            total=len(all_vulnerability_files),
            ...
            for vuln_name, data in all_vulnerability_files.items():
                ...
                detailed_results = self._collect_vulnerability_results(
                    filtered_results, vuln
                )
    ...
    def _collect_vulnerability_results(self, filtered_results, vuln):
        ...
        for file_path, similarity_score in filtered_results:
            ...
            if (
                analysis
                and not analysis.startswith("Error")
                and not analysis.startswith("No results")
            ):
...
class BatchAdaptiveAnalysis:
    ...
    def submit_task(self, task: List[Tuple[str, str, Any]]):
        ...
        if self.cache_manager.has_caching_info(
            file_path, "", vuln_name, AnalysisType.ADAPTIVE
        ):
            if cached_result := self.cache_manager.get_cached_analysis(
                file_path=file_path,
                ...
                logger.info(
                    f"Using cached adaptive analysis for {file_path} - {vuln_name}"
                )
    ...
    def _perform_static_analysis(self):
        ...
        for file_path, vulnerability, threshold in self.pending_tasks:
            try:
                ...
                vuln_name, vuln_desc, vuln_patterns, vuln_impact, vuln_mitigation = (
                    self.analyzer._get_vulnerability_details(vulnerability)
                )
                analysis_cache_key = (
                    f"{sanitize_name(file_path)}_{sanitize_name(vuln_name)}"
                )

                # Skip if result already in cache
                if analysis_cache_key in self.cache_manager.adaptive_analysis_cache:
                    self.results[file_path] = (
                        self.cache_manager.adaptive_analysis_cache[analysis_cache_key]
                    )
                    continue
                ...
                logger.debug(
                    f"PHASE 1: Static pattern analysis for {file_path} - {vuln_name}"
                )
                suspicious_chunks = self.pipeline._static_pattern_analysis(
                    code_chunks, vuln_patterns
                )
                ...
                logger.exception(
                    f"Error during static analysis for {file_path}: {str(e)}"
                )
    ...
    def _perform_scan_model_phases(self):
        ...
        for (file_path, vuln_name), data in scan_tasks:
            try:
                logger.debug(
                    f"PHASE 2: Lightweight model scan for {file_path} - {vuln_name}"
                )
                ...
                logger.exception(
                    f"Error during lightweight scan for {file_path}: {str(e)}"
                )
        ...
        for (file_path, vuln_name), data in scan_tasks:
            ...
            try:
                logger.debug(
                    f"PHASE 3: Context-sensitive analysis for {file_path} - {vuln_name}"
                )
                suspicious_chunks = data["suspicious_chunks"]
                vuln_details = data["vuln_details"]
                ...
                context_sensitive_chunks = (
                    self.pipeline._identify_context_sensitive_chunks(
                        suspicious_chunks, vuln_name
                    )
                )
                self.analysis_data[(file_path, vuln_name)][
                    "context_sensitive_chunks"
                ...
                if context_sensitive_chunks:
                    ...
                    high_risk_chunks = self.pipeline._identify_high_risk_chunks(
                        suspicious_chunks, medium_results
                    )
                    self.analysis_data[(file_path, vuln_name)][
                        "high_risk_chunks"
                    ] = high_risk_chunks
                ...
                logger.exception(
                    f"Error during medium analysis for {file_path}: {str(e)}"
                )
    ...
    def _perform_deep_model_phase(self):
        ...
        for (file_path, vuln_name), data in deep_tasks:
            try:
                ...
                logger.exception(
                    f"Error during deep analysis for {file_path}: {str(e)}"
                )
    ...
    def _generate_final_results(self):
        ...
        for (file_path, vuln_name), data in self.analysis_data.items():
            ...
            try:
                ...
                logger.debug(
                    f"Stored adaptive analysis result in cache for {file_path} - {vuln_name}"
                )

            except Exception as e:
                logger.exception(
                    f"Error combining results for {file_path} - {vuln_name}: {str(e)}"
                )
    ...
    def get_result(self, result_key):
        ...
        return self.results.get(
            result_key, f"No results for {result_key[0]} - {result_key[1]}"
        )
    ...
    def _process_all_tasks_in_batches(self):
        ...
        logger.debug(
            f"PHASE 1: Performing static pattern analysis for {len(self.pending_tasks)} tasks"
        )
    ...
    def _perform_static_analysis_batch(self):
        ...
        with tqdm(
            total=len(self.pending_tasks), desc="Static analysis", leave=False
        ) as pbar:
            for file_path, vulnerability, threshold in self.pending_tasks:
                try:
                    # Extract vulnerability details
                    ...
                    if file_path not in self.code_base:
                        if (
                            result_key not in self.results
                        ):  # Only set if not already set from cache
                            self.results[result_key] = (
                                "File not found in indexed code base"
                            )
                        pbar.update(1)
                        continue
                    ...
                    self.results[(file_path, vuln_name)] = (
                        f"Error during static analysis: {str(e)}"
                    )

                pbar.update(1)

        logger.info(
            f"Cache utilization - Static analysis: {self.cache_stats['static']['cache_hits']}/{self.cache_stats['static']['total']} files ({self.cache_stats['static']['cache_hits']/self.cache_stats['static']['total']*100:.1f}%)"
        )
    ...
    def _perform_scan_model_phases_batch(self):
        ...
        if not self.models_loaded["scan"]:
            logger.info(
                f"Loading scan model {self.ollama_manager.get_model_display_name(self.scan_model)} for all tasks"
            )
            self.ollama_manager.ensure_model_available(self.scan_model)
            self.models_loaded["scan"] = True
        ...
        with tqdm(
            total=len(scan_tasks), desc="Lightweight scanning", leave=False
        ) as pbar:
            for result_key, data in scan_tasks.items():
                try:
                    ...
                    pbar.set_postfix_str(
                        f"File: {Path(file_path).name}, Vuln: {vuln_name}"
                    )
                    ...
                    if has_cached_result:
                        ...
                        logger.debug(
                            f"Using cached result for scan phase: {file_path} - {vuln_name}"
                        )
                    ...
                    logger.exception(
                        f"Error during lightweight scan for {file_path} - {vuln_name}: {str(e)}"
                    )
                    if (
                        result_key not in self.results
                    ):  # Only set if not already set from cache
                        self.results[result_key] = (
                            f"Error during lightweight scan: {str(e)}"
                        )

                pbar.update(1)

        logger.info(
            f"Cache utilization - Lightweight scan: {self.cache_stats['scan']['cache_hits']}/{self.cache_stats['scan']['total']} files ({self.cache_stats['scan']['cache_hits']/self.cache_stats['scan']['total']*100:.1f}%)"
        )
        ...
        with tqdm(
            total=len(scan_tasks), desc="Medium-depth analysis", leave=False
        ) as pbar:
            for result_key, data in scan_tasks.items():
                try:
                    file_path, vuln_name = result_key
                    ...
                    logger.exception(
                        f"Error during medium analysis for {file_path} - {vuln_name}: {str(e)}"
                    )
                    if (
                        result_key not in self.results
                    ):  # Only set if not already set from cache
                        self.results[result_key] = (
                            f"Error during medium analysis: {str(e)}"
                        )
    ...
    def _perform_deep_model_phase_batch(self):
        ...
        if not self.models_loaded["deep"]:
            logger.info(
                f"Loading deep model {self.ollama_manager.get_model_display_name(self.llm_model)} for all tasks"
            )
            self.ollama_manager.ensure_model_available(self.llm_model)
            self.models_loaded["deep"] = True
        ...
        logger.info(
            f"PHASE 4: Deep analysis for {len(deep_tasks)} tasks with high-risk chunks"
        )
            ...
            for result_key, data in deep_tasks.items():
                try:
                    ...
                    if has_cached_result:
                        ...
                        logger.debug(
                            f"Using cached result for deep phase: {file_path} - {vuln_name}"
                        )
                    ...
                    logger.exception(
                        f"Error during deep analysis for {file_path} - {vuln_name}: {str(e)}"
                    )
                    if (
                        result_key not in self.results
                    ):  # Only set if not already set from cache
                        self.results[result_key] = (
                            f"Error during deep analysis: {str(e)}"
                        )

                pbar.update(1)

        logger.info(
            f"Cache utilization - Deep analysis: {self.cache_stats['deep']['cache_hits']}/{self.cache_stats['deep']['total']} files ({self.cache_stats['deep']['cache_hits']/self.cache_stats['deep']['total']*100:.1f}%)"
        )

    def _generate_final_results_batch(self):
        """Generate final results for all processed tasks"""
        with tqdm(
            total=len(self.analysis_data), desc="Generating reports", leave=False
        ) as pbar:
