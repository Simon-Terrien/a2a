...
class SecurityAgentBase(A2AServer):
        ...
        if A2A_AVAILABLE:
            agent_card = AgentCard(
                ...
                description=self.config.get(
                    "description", f"Expert in {agent_type} vulnerabilities"
                ),
                ...
                skills=[
                    AgentSkill(
                        ...
                        examples=[
                            f"Analyze code for {skill.lower()}",
                            f"Detect {skill.lower()} patterns",
                        ],
                    )
    ...
    def start_server(self):
        ...
        if not A2A_AVAILABLE:
            logger.debug(
                f"A2A not available, skipping server start for {self.agent_type}"
            )
            return True
        ...
        try:
            ...
            logger.debug(
                f"✅ Started A2A server for {self.agent_type} on port {self.port}"
            )
            return True
    ...
    def analyze_chunk(
        self, chunk: str, vulnerability: Dict, file_path: str = None
    ) -> str:
        """
...
"""
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            logger.exception(
                f"Error handling A2A message in {self.agent_type}: {str(e)}"
            )
            return {"agent": self.agent_type, "error": str(e), "timestamp": time.time()}
    ...
    def _extract_confidence(self, analysis_result: str) -> float:
        ...
        try:
            ...
            confidence_match = re.search(
                r"confidence.*?(\d+(?:\.\d+)?)", analysis_result.lower()
            )
            if confidence_match:
...
class AgentOrchestrator:
            ...
            if agent_type in agent_classes:
                try:
                    ...
                    if agent.ensure_model_available():
                        ...
                        logger.info(
                            f"✅ {agent.config.get('name', agent_type)} initialized"
                        )
                    else:
    ...
    def _init_caches(self):
        ...
        for agent_type in self.agents.keys():
            cache_manager = CacheManager(
                ...
                scan_model=self.agents[
                    agent_type
                ].model,  # Agents use same model for both
                ...
                for vuln_name, results in agent_results.items():
                    ...
                    if results:
                        report.generate_vulnerability_report(
                            vulnerability=next(
                                v for v in vuln_list if v["name"] == vuln_name
                            ),
                            results=results,
    ...
    def _route_vulnerabilities_to_agents(
    ...
    def _process_agent_vulnerabilities(
        self, agent_type: str, vulnerabilities: List[Dict], silent: bool = False
    ) -> Dict[str, List[Dict]]:
        ...
        for vulnerability in vulnerabilities:
            ...
            if detailed_results:
                ...
                logger.info(
                    f"🎯 {agent_type} found {len(detailed_results)} potential {vuln_name} issues"
                )
    ...
    def _find_vulnerable_files(self, vulnerability: Dict) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            results = self._search_vulnerabilities_embedding(
                vulnerability, self.args.threshold
            )
            return [
                (path, score) for path, score in results if score >= self.args.threshold
            ]
        except Exception as e:
            logger.exception(f"Error finding vulnerable files: {str(e)}")
            return []

    def _search_vulnerabilities_embedding(
        self, vulnerability: Union[str, Dict], threshold: float = 0.5
        ...
        try:
            ...
            if not vuln_vector:
                logger.error(
                    f"Failed to get embedding for vulnerability type '{vuln_name}'"
                )
                return []
    ...
    def _process_functions_embedding(
        self,
        file_path: str,
        data: Dict,
    ...
    def _analyze_file_with_agent(
        self,
        agent: SecurityAgentBase,
        file_path: str,
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                try:
                    analysis_result = agent.analyze_chunk(
                        chunk, vulnerability, file_path
                    )
                    if analysis_result and not analysis_result.startswith("Error"):
                        analyses.append(f"### Chunk {i+1}\n{analysis_result}")
                except Exception as e:
                    logger.debug(
                        f"Error analyzing chunk {i} with {agent.agent_type}: {str(e)}"
                    )
                    continue
            ...
            logger.exception(
                f"Error analyzing {file_path} with {agent.agent_type}: {str(e)}"
            )
            return None

    def _correlate_agent_findings(
    ...
    def _calculate_combined_risk(self, findings: List[Dict]) -> str:
        ...
        amplification = (
            1.0 + (len(findings) - 1) * 0.2
        )  # 20% increase per additional vulnerability
    ...
    def _generate_attack_chains(self, findings: List[Dict]) -> List[str]:
        ...
        chain_patterns = {
            (
                "auth",
                "sqli",
            ): "Authentication bypass → SQL injection → Database compromise",
            ("xss", "auth"): "XSS attack → Session hijacking → Account takeover",
            (
                "config",
                "crypto",
            ): "Configuration exposure → Cryptographic weakness → Data breach",
            (
                "sqli",
                "config",
            ...
            (
                "auth",
                "config",
            ): "Authentication bypass → Configuration access → System compromise",
        }
        ...
        if not attack_chains and len(findings) > 1:
            ...
            attack_chains.append(
                f"Multi-vector attack: {' → '.join(vuln_types)} → System compromise"
            )

        return attack_chains

    def _enhance_with_mcp_tools(
        self, all_results: Dict[str, List[Dict]]
    ) -> Dict[str, List[Dict]]:
            ...
            for result in results:
                try:
                    ...
                    enhanced_result["mcp_enhancements"] = {
                        ...
                        "external_confidence": self._calculate_external_confidence(
                            cve_data, semgrep_data
                        ),
                    }
    ...
    def _calculate_external_confidence(
        self, cve_data: Dict, semgrep_data: Dict
    ) -> float:
    ...
    def save_all_caches(self):
        ...
        for agent_type, cache_manager in self.cache_managers.items():
            try:
                ...
                cache_manager.save_chunk_cache(
                    "", AnalysisMode.SCAN, AnalysisType.COLLABORATIVE
                )
                cache_manager.save_chunk_cache(
                    "", AnalysisMode.DEEP, AnalysisType.COLLABORATIVE
                )
            except Exception as e:
...
class EmbeddingAnalyzer:
            ...
            if vuln_vector:
                for file_path, data in self.code_base.items():
                    if self.analyze_by_function:
                        self._process_functions_for_audit(
                            file_path, data, vuln_vector, results
                        )
                    else:
                        self._process_file_for_audit(
                            file_path, data, vuln_vector, results
                        )
        except Exception as e:
    ...
    def _process_functions_for_audit(
        self, file_path: str, data: Dict, vuln_vector: List[float], results: List[Dict]
    ):
    ...
    def _process_file_for_audit(
        ...
        try:
            ...
            if isinstance(embedding, dict):
                ...
                similarities = [
                    calculate_similarity(vuln_vector, vec) for vec in embedding
                ]
