...
from .utils import create_cache_dir, logger, chunk_content, parse_input, sanitize_name, open_file
...
class EmbeddingManager:
    ...
    def route_to_agents(self, vulnerability_results: Dict[str, List]) -> Dict[str, List]:
        """
    ...
    def get_agent_specific_embedding(self, file_path: str, agent_type: str) -> Optional[List[float]]:
        """
        ...
        if isinstance(entry, dict):
            ...
            if 'embedding' in normalized and isinstance(normalized['embedding'], list):
                if not hasattr(self, 'embedding_dim') or self.embedding_dim is None:
                    ...
                    logger.debug(f"Initialized self.embedding_dim to {self.embedding_dim}")
                elif len(normalized['embedding']) != self.embedding_dim:
                    logger.error(f"Inconsistent embedding dimension: expected {self.embedding_dim}, got {len(normalized['embedding'])}")
    ...
    def save_cache(self):
        ...
        try:
            ...
            if self.multi_agent_mode and self.agent_embeddings:
                agent_cache_file = self.cache_file.parent / f"{self.cache_file.stem}_agents.cache"
    ...
    def get_embeddings_info(self) -> dict:
        ...
        info = {
            ...
            'agent_embeddings': len(self.agent_embeddings) if self.multi_agent_mode else 0,
            'files': {}
        }
        ...
        for file_path in self.code_base:
            ...
            if self.multi_agent_mode and file_path in self.agent_embeddings:
                file_info['agent_embeddings'] = list(self.agent_embeddings[file_path].keys())
