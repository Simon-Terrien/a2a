...
class EmbeddingManager:
        ...
        if isinstance(entry, dict):
            ...
            if "embedding" in normalized and isinstance(normalized["embedding"], list):
                if not hasattr(self, "embedding_dim") or self.embedding_dim is None:
                    ...
                    logger.debug(
                        f"Initialized self.embedding_dim to {self.embedding_dim} based on the first embedding encountered"
                    )
                elif len(normalized["embedding"]) != self.embedding_dim:
                    logger.error(
                        f"Inconsistent embedding dimension: expected {self.embedding_dim}, got {len(normalized['embedding'])} for entry"
                    )
    ...
    def get_embeddings_info(self) -> dict:
        ...
        """
        info = {"total_files": len(self.code_base), "files": {}}
    ...
    def extract_functions_with_regex(
        ...
        try:
            ...
            for i, match in enumerate(matches):
                ...
                func_name = next(
                    (g for g in match.groups() if g), f"anonymous_function_{i}"
                )
    ...
    def extract_functions_with_llm(
        ...
        try:
            ...
            try:
                ...
                for func in result.get("functions", []):
                    ...
                    if (
                        ...
                        and end is not None
                        and start < end
                        and end <= len(normalized_content)
                    ):
                        ...
                        if name in func_content:
                            ...
                            logger.warning(
                                f"Function extraction mismatch for {name} in {file_path}"
                            )
                ...
                logger.exception(
                    f"Invalid JSON in LLM response for {file_path}: {str(e)}"
                )
            ...
            logger.exception(
                f"Error using LLM for function extraction in {file_path}: {str(e)}"
            )
    ...
    try:
        ...
        if args.analyze_by_function:
            ...
            file_embedding = generate_content_embedding(
                content, args.embed_model, args.chunk_size, ollama_manager
            )
