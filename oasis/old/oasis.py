...
class OasisScanner:
    ...
    def setup_argument_parser(self):
        ...
        cache_group = parser.add_argument_group(
            "Cache Management", "Options for managing cache files"
        )
    ...
    def get_vulnerability_help(self) -> str:
        ...
        vuln_list = []
        vuln_list.extend(f"{tag:<8} - {vuln['name']}" for tag, vuln in vuln_map.items())
    ...
    def run_analysis_mode(self, main_models, scan_model, vuln_mapping):
        ...
        vulnerabilities, invalid_tags = SecurityAnalyzer.get_vulnerabilities_to_check(
        ...
        for i, main_model in enumerate(main_models):
            ...
            try:
                ...
                logger.exception(
                    f"Error during security analysis with {main_model}: {str(e)}"
                )
    ...
    def handle_audit_mode(self, vuln_mapping):
        ...
        # Create analyzer
        embedding_manager = EmbeddingAnalyzer(
            self.embedding_manager, self.ollama_manager
        )

        # Analyze all vulnerabilities
        analyzer_results = embedding_manager.analyze_all_vulnerabilities(
            vulnerabilities
        )
    ...
    def _init_arguments(self, args) -> Union[bool, None]:
        ...
        if self.args.version:
            ...
            print(
                f"OASIS - Ollama Automated Security Intelligence Scanner v{__version__}"
            )
            return None
