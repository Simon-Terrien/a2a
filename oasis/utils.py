...
def correlate_agent_findings(findings_by_agent: Dict[str, List[Dict]]) -> Dict[str, Any]:
    """
...
def _generate_attack_chains(agent_findings: List[Dict]) -> List[str]:
    ...
    attack_patterns = {
        ...
        ('config', 'crypto'): "Configuration exposure → Weak cryptography → Data breach",
        ('sqli', 'config'): "SQL injection → Configuration disclosure → Privilege escalation",
        ('crypto', 'config'): "Weak encryption → Configuration access → System compromise"
    }
...
def _calculate_risk_amplification(agent_findings: List[Dict]) -> float:
    ...
    amplification_factor = 1.0 + (len(agent_findings) - 1) * 0.25  # 25% increase per additional vuln
...
def build_attack_chains(related_findings: List[Dict]) -> List[str]:
    ...
    if 'authentication' in str(vulnerability_types).lower():
        if 'sql' in str(vulnerability_types).lower():
            chains.append("1. Bypass authentication → 2. Exploit SQL injection → 3. Extract sensitive data")
        elif 'xss' in str(vulnerability_types).lower():
            chains.append("1. Exploit XSS vulnerability → 2. Steal session tokens → 3. Bypass authentication")
    
    if 'configuration' in str(vulnerability_types).lower():
        if 'crypto' in str(vulnerability_types).lower():
            chains.append("1. Access misconfigured system → 2. Exploit weak cryptography → 3. Decrypt sensitive data")
